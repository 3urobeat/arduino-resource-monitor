...
class LiquidCrystal_PCF8574_base : public Print, public LiquidCrystal_dummy {
  public:     
    ...
    void begin()                                           // 4bit standard
    {
      ...
      delayMicroseconds(41);                               // wait for more than 40�s
      ...
      delayMicroseconds(101);                              // wait for mor than 100�s
...
class LiquidCrystal_PCF8574_custompin : public LiquidCrystal_PCF8574 {
  ...
  protected:
    ...
    // erweitern: abh�ngig von current row  den  en oder en2 pulsen (if col 40 && row 4 )
    // oder besser, den en2 einmal im constructor abh�ngig von 40/4 ander setzen und hier nur mehr abh�ngig von der ROW 
